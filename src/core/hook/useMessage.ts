...
import {App} from "vue";
import {globalPropertiesType} from "@/core/types/message";

export let gp: globalPropertiesType


export function messageSetup(app: App<Element>) {
    gp = {
        /**
         * @description 全局加载层
         ...
         */
        $baseLoading: (index = undefined, text = 'loading...') => {
        ...
        $baseAlert: (content, title = '温馨提示', callback = undefined) => {
            ...
            ElMessageBox.alert(content, title, {
                confirmButtonText: app.config.globalProperties.$t('确定'),
         ...
         */
        $basePrompt: (content, title, callback1, callback2, options) => {
            const defaults = {
                confirmButtonText: app.config.globalProperties.$t('确定'),
                cancelButtonText: app.config.globalProperties.$t('取消'),
            }
            const p = Object.assign({}, defaults, options)
            ElMessageBox.prompt(content, title || app.config.globalProperties.$t('温馨提示'), p)
                .then((value) => {
                    if (callback1) {
                        callback1(value)
                    }
        ...
        $baseConfirm: (
            ...
            ElMessageBox.confirm(
                content,
                title || app.config.globalProperties.$t('温馨提示'),
                {
                    confirmButtonText,
                    cancelButtonText,

...
export const myRegExp = {
    ...
    // 检查字符串是否为合法邮政编码
    isPostcode: function (str: string) {
    ...
    // 判断字符串长度
    isLength: function (str: string, minLength: number, maxLength: number) {
    ...
    isPassword: function passWord(str: string) {
        ...
        return str !== '' && !reg1.test(str) && !reg4.test(str) && (reg.test(str) || reg2.test(str) || reg3.test(str));
    }
